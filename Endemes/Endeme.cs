...
namespace InfoLib.Endemes // THIS NAMESPACE IS A PRIMITIVE!  use only System.* or InfoLib.Testing references
{
    ...
    /// <summary>
    ///      Directions for building an endeme from an input or values, (deprecated?)
    /// </summary>
    ...
    ///      A    F            [T] |T. EndemeFormat          helper class      Ascii text format
    ...
    /// 
    /// 
    ///      Maybe RawSource should go from being a boolean to being a decimal or something
    ///      
    /// 
    ///      production ready
    /// </remarks>
    public class Endeme : IWhileSwitchOnChar, IHaveListMembers
    {
        // ----------------------------------------------------------------------------------------
        //  Members
        //    Because the index is derived information, the authoritative, source and the derived source
        ...
        //  Constructors
        // ----------------------------------------------------------------------------------------
        public Endeme(                                                             ) { Init(null, null                 , false    ); }
        public Endeme(EndemeSet set                                , bool rawSource) { Init(set , null                 , rawSource); }
        public Endeme(EndemeSet set, Dictionary<char,int>    endeme, bool rawSource) { Init(set , IndexToString(endeme), rawSource); }
        public Endeme(EndemeSet set, string                  endeme, bool rawSource) { Init(set , endeme               , rawSource); }
        public Endeme(               string                  endeme, bool rawSource) { Init(null, endeme               , rawSource); }
        public Endeme(EndemeSet set, Dictionary<string,int>  endeme, bool rawSource) { Init(set , IndexToString(endeme), rawSource); }
        public Endeme(char[] arr, EndemeOrderingCreationType eot   , bool rawSource) { _set = null; BuildString(arr, eot); BuildIndex(); Quant = new EndemeQuantification(); RawSource = rawSource; }


        // ----------------------------------------------------------------------------------------
        //  While Switch Char controls (warning: while-switches are not threadsafe)  alpha code
        //
        //    Usage Example:
        //
        //       while (action) switch (action.Step)
        //       {
        //           case 'A' :                 new Log().SendAdminEmails(msg, aspect);                                                                break;
        //           case 'D' :                 Debug.WriteLine(msg);                                                                                  break;
        //           case 'E' :                 new Log().ToLogError(aspect.UserName, prefix, ex, msg, (enFocus + "G").ToCharArray()[0], cmd, aspect); break;
        //           case 'H' : if (ex == null) action.Break();                                                                                        break;
        //           case 'I' : if (ex != null) msg = InnermostExceptionText(ex);                                                                      break;
        //           case 'R' : if (ex != null) throw new ApplicationException(prefix + " " + msg, ex); else throw new ApplicationException(prefix);
        //           case 'Q' :                 action.Break();                                                                                        break;
        //           case 'T' :                 new Log().ToTcLibLogOf(prefix + msg, aspect);                                                          break;
        //           case 'V' : if (ex != null) msg += String.Format(" -- Detail: {0}", ex);                                                           break;
        //       }
        /* ------------------------------------------------------------------------------------- */                          /// <summary>leave the while loop</summary>
        public void Break() { Cursor = Count;   }                                                                            /// <summary>Current position in the WhileSwitch sequence</summary>
        public int  Cursor  { get; private set; }                                                                            /// <summary>current switch value in the sequence</summary>
        public char Step    { get { if (Count > Cursor) { return _string[Cursor]; } else {              return '\r' ; } } }  /// <summary>start,continue or stop running the sequence</summary>
        public bool Run     { get { if (Count > Cursor) { Cursor++; return true ; } else { Cursor = -1; return false; } } }  /// <summary>skip forward a number of steps, (like a GOTO)</summary>
        public void Skip(int fwd) { Cursor += fwd;    }

        // ----------------------------------------------------------------------------------------
        //  String casting and related methods and properties     production ready
        // ----------------------------------------------------------------------------------------
        public static implicit operator Endeme(string str) { return new Endeme(str, true); }
        public static implicit operator string(Endeme en ) { return en.ToString();         }
        ...
        public          char   this[int  i]    { get { if (this != null && !this.IsEmpty && _string.Count > i)  return _string[i]; return '\0';     } } /// <summary>returns position of character, 0 is highest</summary>
        public          int    this[char c]    { get { if (_index.ContainsKey(c))         return _index[c]    ; return  -1  ;                       } } /// <summary>Sets the assumed endeme set size for matching calculations</summary>
        public          int    Capacity        { get {                                                          return  Size; } set { Size = value; } } /// <summary>Returns the number of characteristics in this endeme</summary>
        ...
        public          Endeme Opposite        { get { if (EnSet != null) return new Endeme(EnSet, (EnSet.Straight - this) + Reverse, this.RawSource); else throw new InvalidOperationException("can not reverse an endeme with an unspecified endeme set"); } } /// <summary>Returns the name of the endeme set</summary>
        public          string SetName         { get { if (_set    != null)               return _set.Label   ; return  "" ;                        } }
        // ----------------------------------------------------------------------------------------
        //  Short methods       beta code to production ready
        /* ------------------------------------------------------------------------------------- */                                                                                 /// <summary>Adds a character to the index and the authoritative source _string</summary>
        ...
        public          void   Clear                 (                ) { _string = new List<char>(); _index = new Dictionary<char, int>(); RawSource = true; MultipleUse = false; Quant = new EndemeQuantification(); }
        private         bool   CheckEnSet            (                ) { if (EnSet == null) { throw new NullReferenceException("Endeme set may not be null");    } return true ; }
        ...
        public override int    GetHashCode           (                ) { return FullDesignation.GetHashCode();                                                                   } /// <summary>Adds a value to the quantification of an endeme characteristic</summary>
        public          void   Plus                  (char c , int num) { if ('A' <= c && c <= 'Z') Quant.Raw[c] += num;                                                          } /// <summary>Returns a straight string of just the letters or blank</summary>
        public override string ToString              (                ) { if (IsEmpty || _string == null) return ""; else return (new string(_string.ToArray()));                 } /// <summary>Returns the first n items as a string of letters</summary>
        ...
        public          string Labels                (int    num      ) { return _set.Labels(First(num));                                                                         } /// <summary>Determines if en tgt is completely contained in en src (this), thes sets have to be the same</summary>
        public          bool   Contains              (Endeme enTgt    ) { if (Normalized(enTgt).EnSet == this.EnSet) return Contains(enTgt.ToString());        else return false; } /// <summary>Weird side affects, use very sparingly or not at all</summary><remarks>Weird side affects, use very sparingly</remarks>
        private         Endeme Normalized            (Endeme en       ) { Endeme e = null; if (en.EnSet == null) { e = en.Copy(); e.EnSet = this.EnSet; } else e = en; return e;  } /// <summary>Returns the array of letters</summary>
        public          char[] ToCharArray           ()                 {                           return _string.ToArray();                                                     }
        public          double GenerateCalc_geometric(                ) { return GenerateCalc_geometric(        0.93         );                                                   }
        public          double GenerateCalc_linear   (                ) { return GenerateCalc_linear   (       -100.0 / _size);                                                   }
        public          double GenerateRaw_geometric (                ) { return GenerateRaw_geometric (        0.93         );                                                   }
        public          double GenerateRaw_linear    (                ) { return SetRawQuant           (100.0, -100.0 / _size);                                                   }
        // ----------------------------------------------------------------------------------------
        //  Short methods with loops
        /* ------------------------------------------------------------------------------------- */
        ...
        internal        int    SimpleMatch           (int n, Endeme en) { int    match = 0;     foreach (char c in EnSet.LettersUnsorted) { if (this[c] >= 0 && en[c] >= 0) match += Triangle(n-this[c]) * Triangle(n-en[c]); } return match; } // <summary>Increments the raw value for each characteristic</summary>
        private         Endeme Increment             (                ) {                       foreach (char c in ToCharArray()        ) { Quant.Raw [c]++;                                                                  } return this ; } /// <summary>Decrements the raw value for each characteristic</summary>
        private         Endeme Decrement             (                ) {                       foreach (char c in ToCharArray()        ) { Quant.Raw [c]--; if (Quant.Raw[c] <= 0) Remove(c);                                } return this ; } /// <summary>Generates the Calc values from the endeme string using a geometric slope</summary><param name="geom">usually between 0.0 and 1.0, 0.93 is good</param>
        public          double GenerateCalc_geometric(double geom     ) { double num   = 100.0; foreach (char c in ToCharArray()        ) { Quant.Calc[c] = num; num = num * geom ;                                           } return num  ; } /// <summary>Generates the Calc values from the endeme string using a linear slope</summary><param name="slope">usually negative, -4.55 is a good number</param>
        public          double GenerateCalc_linear   (double slope    ) { double num   = 100.0; foreach (char c in ToCharArray()        ) { Quant.Calc[c] = num; num = num + slope;                                           } return num  ; } /// <summary>Generates the raw values from the endeme string using a geometric slope</summary><param name="geom">usually between 0.0 and 1.0, 0.93 is good</param>
        public          double GenerateRaw_geometric (double geom     ) { double num   = 100.0; foreach (char c in ToCharArray()        ) { Quant.Raw [c] = num; num = num * geom ;                                           } return num  ; } /// <summary>Generates the raw values from the endeme string using a linear slope</summary><param name="slope">usually negative, -4.55 is a good number</param>
        public          double GenerateRaw_linear    (double slope    ) { double num   = 100.0; foreach (char c in ToCharArray()        ) { Quant.Raw [c] = num; num = num + slope;                                           } return num  ; }
        public          double SetRawQuant (double start, double slope) { double num   = start; foreach (char c in ToCharArray()        ) { Quant.Raw [c] = num; num = num + slope;                                           } return num  ; }
        public          EcList CharacteristicList    (                ) { CheckEnSet(); EcList list = new EcList(); foreach (char c in this.ToCharArray()) { if (EnSet.Contains(c)) list.Add(EnSet[c]);                       } return list; }
        // ----------------------------------------------------------------------------------------
        //  Short operators    alpha code to production ready
        /* ------------------------------------------------------------------------------------- */
        public static implicit operator bool(Endeme en         ) { if (en  == null) return false; else return en.Run             ; } /// <summary>WARNING - endeme addition is not associative, use parentheses when you have more than 2 endemes!</summary>
        ...
        public static Endeme operator  -(Endeme lhs, Endeme rhs) { if (lhs == null) return null ; else return lhs.Minus     (rhs); } /// <summary>removes a characteristic from an endeme</summary>
        public static Endeme operator  -(Endeme lhs, char c    ) { if (lhs == null) return null ; else return lhs.Minus     (c  ); } /// <summary>WARNING - endeme multiplication is not associative, use parentheses when you have more than 2 endemes!</summary>
        ...
        {
            bool? nullish = Is.NullMatch(lhs, rhs); if (nullish != null) return nullish.Value;
            else return (lhs.FullDesignation == rhs.FullDesignation);
        }


        private         int Square  (int p) { if (p >= 0) return p*p;                        else return -1*p*p;         }
        internal static int Triangle(int n) { if (n >= 0) return (n*n+n)/2;                  else return (n*n-n)/-2;     }
        internal static int Shifted (int n) { if (n >  1) return 2<<(n-2);  if (n > -2) return n; return -1*(2<<(-2-n)); }
        ...
        {
            char[] cha = rhs.ToCharArray();
            string str = this;
            for (int i = 0; i < cha.Length; ++i) if (!this.Contains(cha[i])) str += cha[i];
            EndemeSet enSet = this.EnSet;
            if (EndemeSet.IsEmpty(EnSet) && rhs != null && !EndemeSet.IsEmpty(rhs.EnSet))
                enSet = rhs.EnSet;
            return new Endeme(enSet, str, RawSource || rhs.RawSource);
        }
        ...
        private Endeme And(Endeme rhs)
        {
            ...
            Endeme endeme = new Endeme(this.EnSet, str, this.RawSource);
            return endeme;
        }
        ...
        public string Contains_ToString(char cha, string strTrue, string strFalse) { if (Contains(cha)) return strTrue; else return strFalse; }
        ...
        public string Contains_ToString(string combo, string strTrue, string strFalse) { if (Contains(combo)) return strTrue; else return strFalse; }
        ...
        private Endeme Difference(Endeme rhs)
        {
            ...
            //  Do the calculation
            // --------------------------------------------------------------------------
            Endeme endeme = new Endeme(this.EnSet, this.RawSource);
        ...
        /// </summary>
        /// <param name="enIndex">index of zero or less means ignore?</param>
        /// <returns></returns>
        /// <remarks>production ready</remarks>
        private static string IndexToString(Dictionary<char,int> enIndex)
        {
            List<char> e = new List<char>(enIndex.Count);
            for (int i = 0; i < 32 + enIndex.Count; ++i) e.Add(' ');
            foreach (char cha in enIndex.Keys)
            {
                if (enIndex[cha] > 0)
                {
                    int pos = 32 - enIndex[cha];
                    e[pos] = cha;
                }
            }
        ...
        /// <!-- IndexToString -->
        /// <summary>
        ///      Converts a dictionary of single character string positions into an Endeme
        /// </summary>
        /// <param name="enIndex">index of zero or less means ignore</param>
        /// <returns></returns>
        private static string IndexToString(Dictionary<string,int> enIndex)
        {
            List<char> e = new List<char>(enIndex.Count);
            for (int i = 0; i < 32 + enIndex.Count; ++i) e.Add(' ');
            foreach (string cha in enIndex.Keys)
            {
                if (cha            == null) throw new FormatException("null key not allowed for endeme index dictionary key");
                if (cha.Trim().Length != 1) throw new FormatException("endeme index dictionary key '"+cha+"'must be a single character string and not whitespace");
                if (enIndex[cha] > 0)
                {
                    int pos = 32 - enIndex[cha];
                    e[pos] = cha.Trim()[0];
        ...
        /// <remarks>production ready</remarks>
        private void Init(EndemeSet set, string endeme, bool rawSource)
        {
            Cursor = -1;
        ...
        private Endeme LeftShift(int num)
        {
            ...
            if (num >= Count) return new Endeme(EnSet, this.RawSource);
            string endeme = this.ToString().Substring(num);
            return new Endeme(EnSet, endeme, this.RawSource);
        }
        ...
        private double FullMatchBest  (Endeme endeme1, Endeme endeme2, int precision) { return            FullMatchList(endeme1, endeme2, precision).Max(); }
        private double FullMatchMedian(Endeme endeme1, Endeme endeme2, int precision) { List<int> match = FullMatchList(endeme1, endeme2, precision); return match[(int)(match.Count/2.0)];        }
        private double FullMatchWorst (Endeme endeme1, Endeme endeme2, int precision) { return            FullMatchList(endeme1, endeme2, precision)[0];    }
        ...
        private Endeme Minus(Endeme rhs)
        {
            ...
            return new Endeme(EnSet, en, this.RawSource);
        }

        // ----------------------------------------------------------------------------------------
        /// <!-- Minus -->
        /// <summary>
        ///      Removes a characteristic from an endeme
        /// </summary>
        /// <param name="c"></param>
        /// <returns></returns>
        private Endeme Minus(char c)
        {
            Endeme en = Copy();
            en._string.Remove(c);
            return new Endeme(EnSet, en, this.RawSource);
        ...
        public Endeme Move(string toFront, string toBack)
        {
            ...
            if (!string.IsNullOrEmpty(toFront)) temp = new Endeme(EnSet, toFront + Regex.Replace(temp, "[" + toFront + "]", ""), this.RawSource);
            if (!string.IsNullOrEmpty(toBack )) temp = new Endeme(EnSet, Regex.Replace(temp, "[" + toBack  + "]", "") + toBack , this.RawSource);
        ...
        /// <param name="obj"></param>
        /// <returns></returns>
        public static Endeme Of(object obj, bool rawSource)
        {
            ...
            if (typ == typeof(IAmAnEndemeItem))
            {
                ...
                return new Endeme(item.EnSet, item.Item.ToString(), rawSource);
            }

            if (typ == typeof(Endeme)) return (Endeme)obj;
            string endeme = obj.ToString();
            return new Endeme(endeme, rawSource);
        }
        ...
        private Endeme Or(Endeme rhs)
        {
            ...
            return new Endeme(this.EnSet, str, this.RawSource);
        }
        ...
        {
            Endeme en = new Endeme(this.EnSet, this.RawSource);
            foreach (char c in "ABCDEFGHIJKLMNOPQRSTUVWXYZ".ToCharArray())
        ...
        /// <param name="rhs"></param>
        /// <returns></returns>
        public Endeme Times(Endeme rhs)  // TAG
        {
            ...
            // --------------------------------------------------------------------------
            Endeme output = new Endeme(this.EnSet, this.RawSource);
        ...
        /// <returns></returns>
        /// <remarks>alpha code</remarks>
        private Endeme Times_old(Endeme rhs)  // TAG
        {
            // --------------------------------------------------------------------------
            //  Standard checks
            // --------------------------------------------------------------------------
        ...
        /// <returns></returns>
        internal bool HasOrderOf(Endeme order)
        {
            char[] cha = order.ToCharArray();


            bool inOrder = true;
            for (int i = 1; inOrder && i < cha.Length; ++i)
            {
                if (this[cha[i-1]] > this[cha[i]]) inOrder = false;
            }
