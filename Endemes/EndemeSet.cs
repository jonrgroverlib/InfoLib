...
namespace InfoLib.Endemes // THIS NAMESPACE IS A PRIMITIVE!  use only System.* or InfoLib.Testing references
{
    ...
    ///      production ready
    ///      
    ///      If a set has an EndemeSetId then it is hooked to a database
    /// </remarks>
    public class EndemeSet : IHaveListMembers
    {
        // ----------------------------------------------------------------------------------------
        //  Basics
        // ----------------------------------------------------------------------------------------
        public  Dictionary<char, EndemeCharacteristic> Characteristic { get { return _char; } private set { _char = value; } } // could the be a HashSet?
        private Dictionary<char, EndemeCharacteristic> _char;                                                                  // could the be a HashSet?
   //     private int _maxCapacity;
        ...
        [XmlAttribute()] public string Code      { get { return EndemeSetCode    ; } set { EndemeSetCode     = value; } } /// <summary>The name of the endeme set, does it make sense to use this natural key?</summary>
        ...
        [XmlAttribute()] public string SetDescr  { get { return EndemeSetDescr   ; } set { EndemeSetDescr    = value; } }
                         public Endeme EnDefault { get { return new Endeme(this, DefaultEndeme, true); } set { if (value != null) DefaultEndeme = value; else DefaultEndeme = null; } } /// <summary>The resource of the endeme set - what the characetristics are balanced within. I do'nt know if this makes any sense to keep</summary>
        ...
        // ----------------------------------------------------------------------------------------
        //  Accessors (sets have no order so an integer based accessor is pointless)
        // ----------------------------------------------------------------------------------------
        public EndemeCharacteristic this[char   c  ] { get { if (_char.ContainsKey(c)) { _char[c].EndemeSetId = EndemeSetId; return _char[c]; } else return null; } }
        public EndemeCharacteristic this[string lbl] { get { foreach (char c in _char.Keys) { if (_char[c].Label == lbl)     return _char[c]; }      return null; } }  // I wonder if this should use Regex?
        ...
        public EndemeSet(           string label             , bool orderImportant = true) { Init(Guid.Empty, label, "", orderImportant);              }
        public EndemeSet(Guid guid, string label             , bool orderImportant = true) { Init(guid      , label, "", orderImportant);              }
        public EndemeSet(           string label, string code, bool orderImportant = true) { Init(Guid.Empty, label, "", orderImportant); Code = code; }
        ...
        // ----------------------------------------------------------------------------------------
        public          int        Capacity              { get { return 24;                                    } set { throw new NotImplementedException(); } }
        public          void       Clear        ()             { Init(Guid.Empty, ""   , "", false);                                                        }
        ...
        public          void       Pause        ()             {                                                                                            }
        public          void       Sort         ()             { EnDefault = Straight;                                                                      }
        public          char[]     UnsortedAlphabet      { get { return LettersUnsorted.ToArray();                                                          } }
        public          EndemeSet  WithId       (Guid   setId) { SetId = setId; EndemeSetId = setId; return this;                                           }

        public static   bool operator !=(EndemeSet lhs, EndemeSet rhs) { return !(lhs == rhs); }
        public static   bool operator ==(EndemeSet lhs, EndemeSet rhs) { bool? nullCheck = Is.NullMatch(lhs,rhs); if (nullCheck != null) return nullCheck.Value; else return lhs.Equals(rhs); }
        ...
        public EndemeSet Add(EndemeCharacteristic characteristic)
        {
        ...
        /// <!-- DefaultCode -->
        /// <summary>
        ///      Attempts to convert a label to a code of up to 8 upper case letters
        /// </summary>
        /// <param name="label"></param>
        /// <returns></returns>
        public static string DefaultCode(string label)
        {
            string code = "";
            try
            {
                ...
                if (Regex.IsMatch(label, "$[^ ].......[^ ] "))
                     { code = (Regex.Replace(label, " ", "").ToUpper() + "                ").Substring(0, 8).Trim(); }
                else if (label.Length > 8)
                {
                    code = Regex.Replace(label, "^.* ([^ ]+)$", "$1");
                    if (code.Length < 8)
                    {
                        code = label.Substring(0, 8-code.Length) + code;
                    }
                }
                else { code = label; }
        ...
        public Endeme RandomEndeme(int length, Random r)
        {
            ...
            return (new Endeme(this, endeme.ToString(), false));
        }
        ...
        public Endeme Straight
        {
            get
            {
                ...
                foreach (char cha in _char.Keys)
                    str += cha;
                Endeme e = new Endeme(this, str, false);
                return e;
            }
        }
