...
namespace InfoLib.Endemes // THIS NAMESPACE IS A PRIMITIVE!  use only System.* or InfoLib.Testing references
{
    ...
    public class EndemeField
    {
        ...
        //  Accessors
        // ----------------------------------------------------------------------------------------
        public EndemeValue this[EndemeTermKey profile] { get { return GetValue(profile); } set { SetValue(profile, value, true); } }
        ...
        public string AsciiValue { get
        {
            ...
            for (int i = 0; i < ListField.Count; ++i)
            {
                ...
                if (item.Item != null && item.Item.GetType() == typeof(Endeme))
                    str.Append(delim + "[" + item.ToProfileString() + ((Endeme)item.Item.Value).FullDesignation + "]");
        ...
        {
            if (Regex.IsMatch(profile.ToString(), "[*!]")) return RegField .Educe(profile, rawSource);
            else                                return ListField.Educe(profile, rawSource);
        }
        ...
        public EndemeValue GetValue(EndemeTermKey profile)
        {
            ...
            else  return ListField.Educe(profile, true).Item;
        }
        ...
        public void SetValue(EndemeTermKey profile, EndemeValue value, bool rawSource)
        {
            ...
            else ListField.Educe(profile, rawSource).Item = value;
        }
