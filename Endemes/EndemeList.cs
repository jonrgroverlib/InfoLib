...
namespace InfoLib.Endemes // THIS NAMESPACE IS A PRIMITIVE!  use only System.* or InfoLib.Testing references
{
    ...
    ///                   +-------+-+-----+-------+  |               |                         |       |                                +-------------------+  |  |  |  |                 |                           
    ///       _   |                 |                |   |    |      |                         |       |                  |    |       /                       |  |  |  |                 |               |   _     _ 
    ///        |              IEndemeActuatorFactor  |               |  EndemeFormat           |       |  EndemeGrabBag          (endeme index table)          |  |  |  |                 |                  |       |
    ///        |  |                 |                |   |    |      |       |                /|\      |       |          |    |                               |  |  |  |                 |               |  |       |
    ///        |                    +--------------------------------+-------+--------------+--+---------------+------------------------+----------------------+  |  |  |                 |                  |       |
    ///        |  |                                  |   |    |                             |          |                  |    |       /                          |  |  |                 |               |  |       |
    ...
    /// </remarks>
    public class EndemeList : IEnumerable<Guid>, IHaveListMembers //, IQueryable
    {
        // ----------------------------------------------------------------------------------------
        ...
        public  EndemeList this[string label] { get { if (_labelIdx.ContainsKey(label)) return SubList(label)  ; else return null; } }
        public  EndemeItem this[EndemeTermKey enTerm]
        { get { EndemeItem item = GetExactField(enTerm); if (item == null) item = SetFieldExactly(enTerm, new EndemeValue(null)); return item; }
          set { EndemeItem item = GetExactField(enTerm); if (item == null) item = SetFieldExactly(enTerm, value); else item.Item = value.Item; }
        }
        ...
        // ----------------------------------------------------------------------------------------
        public  void              ChangeGuid(Guid oldId, Guid newId) { EndemeItem item = _list[oldId]; Remove(oldId); Add(newId, item);                            }
        public  int               Capacity                     { get {                   return _order.Capacity; } set { throw new NotImplementedException();      } }  /// <summary>Leaves the label and the endeme set intact</summary>
        public  void              Clear               (            ) { _list = new Dictionary<Guid,EndemeItem>(); _order = new List<Guid>(); _labelIdx = new Dictionary<string,List<Guid>>(); }
        ...
        {
            if (string.IsNullOrWhiteSpace(Label))              return "[no label]" + " (" + Count + ")";
            else if (DefaultEnSet == null ||
                string.IsNullOrWhiteSpace(DefaultEnSet.Label)) return        Label + " (" + Count + ")";
            else                           return DefaultEnSet.Label + ":" + Label + " (" + Count + ")";
        }
        ...
        public Guid Add(string label             , string endeme, object value              , bool rawSource) { return    Add(new EndemeItem(label, new Endeme(DefaultEnSet, endeme, rawSource), new EndemeValue(value))); } //return Add(new EndemeItem(label, DefaultEnSet, endeme, new EndemeValue(value))); } // --+  |
        public Guid Add(string label, string code, string endeme, object value, double match, bool rawSource) { Guid id = Add(new EndemeItem(label, new Endeme(DefaultEnSet, endeme, rawSource), new EndemeValue(value))); this[id].ItemCode = code; this[id].TempMatch = match; return id; }
        ...
        public string AsciiValue { get
        {
            ...
            if (Count > 250)
            {
                ...
                //  Build a large list
                // ----------------------------------------------------------------------
                StringBuilder build = new StringBuilder(200 * AsciiItem(0).Length);
                for (int i = 0        ; i < 100  ; ++i) { build.Append(delim + AsciiItem(i)); delim = "\r\n"; } build.Append(delim + "<snip "+(Count-200).ToString()+" items>");
                for (int i = Count-100; i < Count; ++i)   build.Append(delim + AsciiItem(i));
                str = build.ToString();
            }
            else if (Count > 0)
            {
                ...
                //  Build a small list
                // ----------------------------------------------------------------------
                for (int i = 0; i < this.Count; ++i) { str += delim + AsciiItem(i); delim = "\r\n"; }
            }
        ...
        /// </remarks>
        private EndemeList SubList(string label)
        {
            EndemeList output = new EndemeList(label, EnRef, EqualityThreshold);
            List<Guid> guid = _labelIdx[label];
            for (int i = 0; i < guid.Count; ++i)
                output.Add(this[guid[i]]);
            return output;
        }
