...
namespace InfoLib.Endemes // THIS NAMESPACE IS A PRIMITIVE!  use only System.* or InfoLib.Testing references
{
	...
	/// <!-- EndemeFormat -->
    /// <summary>
    ///      The EndemeFormat class implements endeme string formatting
    /// </summary>
    /// <remarks>alpha code</remarks>
    public static class EndemeFormat
    {
        // ----------------------------------------------------------------------------------------
        /// <!-- FourEndemes -->
        /// <summary>
        ...
        public static string Show(Endeme en, int num, int width, string divider, int wordLen, string delim)
        {
            ...
            {
                str = en; // new string(_string.ToArray());
                ...
                switch(width - num)
                {
                    ...
                    case 1 : case 2 :
                        ...
                        else letters.Append(EndemeFormat.SetLength(num, str));
                        break;
                    case 3 :
                        switch(num)
                        {
            ...
            if (en.EnSet != null && width > letters.Length + 5)
            {
                ...
                {
                    letters.Append(d + EndemeFormat.SetLength(wordLen, en.EnSet.Characteristic[en[i]].Label));
                    d = delim;
                }
                d = divider;
                for (int i = Math.Max(0, en.Count-3); i < en.Count; ++i)
                {
                    ...
                    letters.Append(d + EndemeFormat.SetLength(wordLen, addme));
                    d = delim;
                }
                d = divider;
            ...
            return EndemeFormat.SetLength(width, letters.ToString());
        }
        ...
        public static string Show_complex(Endeme en, int width, int divs, string divider)
        {
            ...
            // --------------------------------------------------------------------------
            if (width <= 8)      return EndemeFormat.Show_positive(en, width, divs, num, divider);
            if (num < en.Length) return EndemeFormat.Show_partial (en, width, divs, num, divider);
            else                 return EndemeFormat.Show_perfect (en, width, divs, num, divider);
        }
