...
//--------------------------------------------------------------------------------------------------
using InfoLib.Endemes.Tree;           // for 
using InfoLib.Endemes ;               // for EndemeItem
...
namespace InfoLib.Info
{
    ...
    /// <!-- InfoTests -->
    /// <summary>
    ///      The InfoTests class tests the classes in the AccessTests Library
    /// </summary>
    /// <remarks>
    ...
    public class InfoTests
    {
        ...
        /// <!-- ConnectedTests -->
        /// <summary>
        ///      Tests the Info folder in ways that need a connection
        /// </summary>
        /// <returns></returns>
        public string ConnectedTests(string conn)
        {
        	...
        	// ---------------------------------------------------------------------------
        	//  Info folder tests needing a connection
        	// ---------------------------------------------------------------------------
          //BuildSchema_Test                  ();
          //BuildDatabase_Test                ();
        ...
        /// <!-- UnconnectedTests -->
        /// <summary>
        ///      Tests the Info folder in ways that do not need a connection
        /// </summary>
        /// <returns></returns>
        public string UnconnectedTests()
        {
        	...
        	// ---------------------------------------------------------------------------
        	//  Info folder tests not needing a connection
        	// ---------------------------------------------------------------------------
        ...
        private void EndemeValueAddStrValue_test()
        {
            ...
            field.Add("HeroName"  , "Identity"   , "A" , "", false);
            field.Add("Species"   , "Demographic", "R" , "", false);
            field.Add("Gender"    , "Demographic", "G" , "", false);

            field.Add("First Name", "Identity"   , "F" , "", false);
            field.Add("Last Name" , "Identity"   , "L" , "", false);
            field.Add("Full Name" , "Identity"   , "LF", "", false);
		...
		public void InfoPath_test()
		{
			...
			EndemeUnit leaf1 = new EndemeUnit(new Endeme(InfoTests.Identity, "FN", false));
			leaf1.Text = "Jon";
			EndemeUnit leaf2 = new EndemeUnit(new Endeme(InfoTests.Contact, "EF", false));
			leaf1.Text = "jonrgrover@gmail.com";
			EndemeUnit leaf3 = new EndemeUnit(new Endeme(InfoTests.Address, "MC", false));
        ...
        private void EndemeLeaf_Constructor_test()
        {
            ...
            Endeme endeme = InfoTests.WetlandAnimals.RandomEndeme();
            EndemeUnit item = new EndemeUnit(endeme);
            item.Label = "hi";
            item.Number = 7;
            item.Text = "fosdfjej";

            Assert.That(item.Endeme.ToString(), Is.equal_to, endeme);
        ...
        public static void Knowledge_test()
        {
            ...
            knowledge.Enter("/Identity:1-F(1)"  , "hi world F", false);
            knowledge.Enter("/Identity:.09-G(1)", "hi world G", false);
            knowledge.Enter("/Identity:H(1)"    , "hi world H", false);
            knowledge.Enter("/Identity:I"       , "hi world I", false);
            knowledge.Enter("/Identity:1-J"     , "hi world J", false);
            knowledge.Enter("/Identity:.09-K"   , "hi world K", false);

            EndemeHierarchy each1 = knowledge.QueryOne("/Identity:1-G(1)", false);
            ReflectorTests.Equality("EndemeCollection", "QueryOne", "/Identity:1-G(1)", each1.Value, "hi world G");


            knowledge.Enter("/story part:.9-GTV(1)/identity:1-F(1)", "Gnoephoe", false);
            EndemeHierarchy each2 = knowledge.QueryOne("/story part:.9-GTV(1)", false);
            ...
            EndemeHierarchy each3 = knowledge.QueryOne("/story part:.9-GTV(1)/identity:1-F(1)", false);
            ...
            knowledge.Enter("/story part:.9-GTV/identity:1-L(1)"   , "Brew"            , false); // B  Band of X            membership    group of X
                                                                                          // C  Competetor of X      rivalry       rival
            knowledge.Enter("/story part:1-VRPAGT/identity:1-F(1)" , "Pashalia"        , false); // D  Destroyer of X       destruction   killer of X
            knowledge.Enter("/story part:1-VRPAGT/identity:1-L(1)" , "au Taq"          , false); // E  Enemy of X           evil          villain
                                                                                          // F  Fighter              combat        combatant / fighting
            knowledge.Enter("/story part:1-RPVAGT/identity:1-H(1)" , "Queen"           , false); // G  Guardian of X        prevention    preventer of X
            knowledge.Enter("/story part:1-RPVAGT/identity:1-F(1)" , "Chelaune"        , false); // H  Hero of X            goodness      good
            knowledge.Enter("/story part:1-RPVAGT/identity:1-L(1)" , "au Taq"          , false); // I  Investigator of X    investigation detective / inquiry / query / suspect / possible
                                                                                          // J  Junk / object        owner         thing / owner / McGuffin / item
            knowledge.Enter("/story part:1-PRVEGTH/identity:1-H(1)", "King"            , false); // K  Knows / witness of X knowledge     knowledge belief / witness of X / saw
            knowledge.Enter("/story part:1-PRVEGTH/identity:1-F(1)", "Mekton"          , false); // L  Local                non traveling	
            knowledge.Enter("/story part:1-PRVEGTH/identity:1-L(1)", "au Taq"          , false); // M  Monster              monstrous     creature
                                                                                          // N  Next X               subsequent    future x
            knowledge.Enter("/story part:1-LOFHAP/identity:1-F(1)" , "Zelda"           , false); // O  Official of X        leadership    leader of X
            knowledge.Enter("/story part:1-LOFHAP/identity:1-P(1)" , "The Amazon Queen", false); // P  Patron of X          patronage     client / customer
                                                                                          // Q  Quester of X         traveling     company / party / traveler
            knowledge.Enter("/story part:1-FAH/identity:1-F(1)"    , "Lida"            , false); // R  Related to X         family	
                                                                                          // S  Spy / scout          secret	
            knowledge.Enter("/story part:1-ARGT/identity:1-F(1)"   , "Ilso"            , false); // T  Thief / kidnapper    taker         stealer / capturer of X
            knowledge.Enter("/story part:1-ARGT/identity:1-L(1)"   , "Brew"            , false); // U  Unknowing            unknown       unbelieving / nonbeliever / ignorant / amnesia / sceptic
                                                                                          // V  Victim               missing       lost / damsel
            knowledge.Enter("/story part:1-OAPSGT/identity:1-H(1)" , "Page"            , false); // 
            knowledge.Enter("/story part:1-OAPSGT/identity:1-F(1)" , "Jeffrey"         , false); // 
                                                                                          // 
            knowledge.Enter("/story part:1-AGTPRLH/identity:1-H(1)", "Captain"         , false); // 
            knowledge.Enter("/story part:1-AGTPRLH/identity:1-F(1)", "Bravard"         , false); // 
            knowledge.Enter("/story part:1-AGTPRLH/identity:1-A(1)", "Barvard"         , false); // 

            knowledge.Enter("/player/Identity:1-F(1)"   , "Jon"     , false);
            knowledge.Enter("/player/Identity:1-L(1)"   , "Grover"  , false);
