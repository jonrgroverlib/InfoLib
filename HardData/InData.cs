...
namespace InfoLib.HardData // THIS NAMESPACE IS A PRIMITIVE!  use only System.* or InfoLib.Testing references
{
    ...
    public class InData
    {
            ...
            try
            {
                ...
                foreach (SqlParameter param in command.Parameters)
                {
                    ...
                    switch (typeName)
                    {
                        ...
                        default         : query += delim + " DECLARE " + paramName + " AS" + " " +  typeName      .PadRight(17) + ";"; break;
                    }
        ...
        /// <!-- CountOf -->
        /// <summary>
        ///      Returns a count of the rows that match the where clause filter,
        ///      WARNING, SQL injection danger, don't just drop stuff in here
        /// </summary>
        /// <param name="tableName"></param>
        /// <param name="whereClause"></param>
        /// <param name="connection"></param>
        /// <returns></returns>
        public static int CountOf(string fromWhereClause, SqlConnection connection)
        {
            ...
            return count;
        } /// <summary>Returns a count of the rows that match the where clause filter, WARNING, SQL injection danger, don't just drop stuff in here</summary>
        public static int CountOf(string fromWereClause, string conn) { SqlConnection connection = ConnectSource.Connection(conn); connection.Open(); try { return CountOf(fromWereClause, connection); } catch (Exception ex) { Is.Trash(ex); } finally { connection.Close(); } return 0; }
        ...
        public static int DeleteUpTo(int maxDeleteCount, string fromWhereClause, string conn)
        {
            ...
            using (SqlConnection connection = new SqlConnection(conn))
                { connection.Open();  count = InData.DeleteUpTo(maxDeleteCount, fromWhereClause, connection);  connection.Close(); }
            return count;
        }
        ...
        // ----------------------------------------------------------------------------------------
        /// <!-- ResistSqlInjection_lite -->
        /// <summary>
        ///      A fairly simple attempt to resist sql injection in dynamic queries, allows some sql through however
        /// </summary>
        /// <param name="str"></param>
        /// <returns></returns>
        /// <remarks>production ready</remarks>
        public static string ResistSqlInjection_lite(string str)
        {
            string output = str;

            output = Regex.Replace(output, "(/[*]|[*]/)"                                                 , "");
            output = Regex.Replace(output, "VARCHAR[(]MAX[)]"                                            , "Keep4585", RegexOptions.IgnoreCase);
            ...
            output = Regex.Replace(output, "(alter|create|drop|truncate)(.*)(role|table|view)"           , "'$1'$2$3", RegexOptions.IgnoreCase | RegexOptions.Singleline);
            output = Regex.Replace(output, "(grant|revoke)(.*)(insert|select|update|delete|all)"         , "'$1'$2$3", RegexOptions.IgnoreCase | RegexOptions.Singleline);
            output = Regex.Replace(output, "(delete)(.*)(from|login|name|top|user|[(][)])"               , "'$1'$2$3", RegexOptions.IgnoreCase | RegexOptions.Singleline);
            output = Regex.Replace(output, "(select)(.*)(login|user)"                                    , "'$1'$2$3", RegexOptions.IgnoreCase | RegexOptions.Singleline);
            output = Regex.Replace(output, "(insert|update)(.*)(into|set|values)"                        , "'$1'$2$3", RegexOptions.IgnoreCase | RegexOptions.Singleline);
            output = Regex.Replace(output, "(order) +(by)"                                               , "'$1$2'"  , RegexOptions.IgnoreCase | RegexOptions.Singleline);
            output = Regex.Replace(output, "(cmd|exec|execute)(.*)(cmd|dbo|execute|master|sys|xp_|sp_)"  , "'$1'$2$3", RegexOptions.IgnoreCase | RegexOptions.Singleline);
            output = Regex.Replace(output, "(;)(.*)(shutdown)"                                           , "'$1'$2$3", RegexOptions.IgnoreCase | RegexOptions.Singleline);
            ...
            output = Regex.Replace(output, "Keep4585", "VARCHAR(MAX)");
        ...
        public static int UpdateUpTo(int maxRows, string tableName, string whereClause, string column, object value, SqlConnection connection)
        {
            ...
            // --------------------------------------------------------------------------
            int count = CountOf("FROM " + tableName + " " + whereClause, connection);
        ...
        public static bool ValueExists(DataRow field, string csvColumn, string tableName, string dbColumn, SqlConnection connection)
        {
            ...
            {
                DataTable table = InData.GetTable(tableName, "SELECT * FROM " + tableName + " WITH(NOLOCK) WHERE "+dbColumn+" = " + value.ToString(), connection);
