...
//--------------------------------------------------------------------------------------------------
using InfoLib.Testing;         // for Assert, Is
using System;                         // for 
    ...
    public class HardDataTests
    {
	  ...
	  //private static string _conn;
        ...
        public void InData_DetectSqlInjection_test()
        {
            ...
            InData_DetectSqlInjection_testcase("-1 UNION SELECT 1,pass,cc FROM users WITH(NOLOCK) WHERE uname='test'"   , true);
            InData_DetectSqlInjection_testcase("a';DROP TABLE users; SELECT * FROM userinfo WITH(NOLOCK) WHERE 't' = 't", true);
