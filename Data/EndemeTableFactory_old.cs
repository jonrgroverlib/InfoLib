...
using InfoLib.SoftData;               // for TreatAs.IntValue
using InfoLib.Strings ;               // for __.DoubleQuotes, __.Truncate
using System          ;               // for 
	...
	public class EndemeTableFactory_old
	{
		...
		public static void BuildEndemeCharacteristicTable(string conn)
		{
			RichSqlCommand cmd = new RichSqlCommand(CommandType.Text, conn
                , "\r\n" + ""
                + "\r\n" + " CREATE TABLE "+EndemeAccess.LaEndemeChar+"("
		...
		public static void BuildEndemeIndexTable(string conn)
		{
			RichSqlCommand cmd = new RichSqlCommand(CommandType.Text, conn
                , "\r\n" + ""
                + "\r\n" + " CREATE TABLE "+EndemeAccess.LaEndemeIndex+"("
		...
		public static void BuildEndemeMeaningTable_old(string conn)
		{
			RichSqlCommand cmd = new RichSqlCommand(CommandType.Text, conn
                , "\r\n" + ""
                + "\r\n" + " CREATE TABLE "+EndemeAccess.LaEndemeMeaning
		...
		public static void BuildEndemeSetTable(string conn)
		{
            ...
            {
				RichSqlCommand cmd = new RichSqlCommand(CommandType.Text, conn
                    , "\r\n" + " CREATE TABLE "+EndemeAccess.LaEndemeSet+"("
		...
		{
			RichSqlCommand cmd = new RichSqlCommand(CommandType.Text, conn
                , "\r\n" + " CREATE TABLE "+EndemeAccess.LaEndemeMain+"("
		...
		{
			RichSqlCommand cmd = new RichSqlCommand(CommandType.Text, conn, " DROP TABLE "+EndemeAccess.LaEndemeMain+";"
		...
		{
			RichSqlCommand cmd = new RichSqlCommand(CommandType.Text, conn
                , " DROP TABLE "+EndemeAccess.LaEndemeMain   +";"
                + " DROP TABLE "+EndemeAccess.LaEndemeChar   +";"
                + " DROP TABLE "+EndemeAccess.LaEndemeIndex  +";"
                + " DROP TABLE "+EndemeAccess.LaEndemeSet    +";"
                + " DROP TABLE "+EndemeAccess.LaEndemeMeaning+";"
                , Throws.Actions, exceptionAction);
			cmd.ExecuteNonQuery();
		}
        ...
        {
            RichSqlCommand cmd = new RichSqlCommand(CommandType.Text, conn
                , "SELECT SetId FROM "+EndemeAccess.LaEndemeSet + " WITH(NOLOCK) WHERE Label = '"+setName+"'"
                , Throws.Actions, "P");
            RichDataTable table = new RichDataTable(cmd, "OneSet", SetIdColumn);
		...
		private static EndemeSet GetEndemeSet_old(Guid setId, string conn)
		{
			...
			RichSqlCommand cmd = new RichSqlCommand(CommandType.Text, conn
                ...
                + "\r\n" + " FROM           " + EndemeAccess.LaEndemeChar + " AS c WITH(NOLOCK)"
                + "\r\n" + "     INNER JOIN " + EndemeAccess.LaEndemeSet  + " AS s WITH(NOLOCK) ON s.SetId = c.SetId"
                + "\r\n" + " WHERE   s.SetId   = '" + setId.ToString() + "'"
                , Throws.Actions, "P");
			RichDataTable setCharacteristic = new RichDataTable(cmd, "SetCharacteristic", SetIdColumn);
        ...
        public static EndemeSet GetMostRecentEndemeSet_old(string setName, string conn)
        {
            ...
            RichSqlCommand cmd = new RichSqlCommand(CommandType.Text, conn
                ...
                + " FROM                " + EndemeAccess.LaEndemeChar + " AS c WITH(NOLOCK)"
                + "     LEFT OUTER JOIN " + EndemeAccess.LaEndemeSet  + " AS s WITH(NOLOCK) ON s.SetId = c.SetId"
                + " WHERE s.SetId IN ("
                ...
                + " ORDER BY Letter"
                , Throws.Actions, "IP");
            RichDataTable table = new RichDataTable(cmd, "Characteristic", "EndemeCharacteristicID");
		...
		public static EndemeSet GetEndemeSet_old(string conn, string setName, string tableName, string charColumn, string labelColumn, string descrColumn)
		{
			RichSqlCommand cmd = new RichSqlCommand(CommandType.Text, conn
                ...
                , Throws.Actions, "P");
			return new RichDataTable(cmd, tableName, "").ToEndemeSet(setName, charColumn, labelColumn, descrColumn);
		}
		...
		public static EndemeSet GetEndemeSet_old(string conn, string setName, string setVersion)
		{
			...
			RichSqlCommand cmd = new RichSqlCommand(CommandType.Text, conn
                ...
                + " FROM           " + EndemeAccess.LaEndemeChar + " AS c WITH(NOLOCK)"
                + "     INNER JOIN " + EndemeAccess.LaEndemeSet  + " AS s WITH(NOLOCK) ON s.EndemeSetId = c.EndemeSetId"
                + " WHERE   s.Label   = '" + setName + "'"
                + "     AND s.Version = '" + setVersion + "'"
                , Throws.Actions, "P");
			RichDataTable setCharacteristic = new RichDataTable(cmd, "SetCharacteristic", "EndemeCharacteristicID");
		...
		private static RichDataTable GetEndemeSetHeader_old(string conn, string setName, string setVersion)
		{
			...
			// ---------------------------------------------------------------------------
			RichSqlCommand cmd = new RichSqlCommand(CommandType.Text, conn
                , " SELECT *" + " FROM   "+EndemeAccess.LaEndemeSet + " AS s WITH(NOLOCK)"
                + " WHERE  s.Label   = '" + setName + "'"
			...
			//  Get the knowledge table
			// ---------------------------------------------------------------------------
			return new RichDataTable(cmd, "EndemeSet", SetIdColumn);
		}
        ...
        {
            RichSqlCommand cmd = new RichSqlCommand(CommandType.Text, conn
                , "SELECT * FROM "+EndemeAccess.LaEndemeSet + " WITH(NOLOCK) WHERE Label = '"+setName+"'"
                , Throws.Actions, "P");
            RichDataTable table = new RichDataTable(cmd, "OneSet", SetIdColumn);
		...
		private static Guid InsertCharacteristic_old(EndemeCharacteristic characteristic, Guid setId, string conn)
		{
			...
			// ---------------------------------------------------------------------------
			RichSqlCommand cmd = new RichSqlCommand(CommandType.Text, conn
                , "\r\n" + " INSERT INTO "+EndemeAccess.LaEndemeChar+""
                + "\r\n" + "        ( CharId,  SetId,  Letter,  Code,  Label,  Descr)"
                + "\r\n" + " VALUES (@CharId, @SetId, @Letter, @Code, @Label, @Descr)"
                , Throws.Actions, "P")
                ._AddParameter     ("@CharId", charKey                    )
                ._AddParameter     ("@SetId" , setId                      )
                ._AddParameter     ("@Letter", characteristic.Letter      )
                ._AddParameter_safe("@Code"  , characteristic.Code  ,   16)
                ._AddParameter_safe("@Label" , characteristic.Label ,  128)
                ._AddParameter_safe("@Descr" , characteristic.Descr , 1024);
            cmd.ExecuteNonQuery();
        ...
        public static Guid InsertEndemeSet_old(EndemeSet setName, string conn)
        {
            ...
            // --------------------------------------------------------------------------
            RichSqlCommand cmd = new RichSqlCommand(CommandType.Text, conn
                , "INSERT INTO "+EndemeAccess.LaEndemeSet + " (SetId, Label, VersionNumber)"
                + " VALUES"
                + "     ( '" + setId.ToString() + "'"
            ...
            foreach (EndemeCharacteristic characteristic in setName.Characteristics())
            {
                ...
                RichSqlCommand cmd2 = new RichSqlCommand(CommandType.Text, conn
                    , "INSERT INTO "+EndemeAccess.LaEndemeChar + " (CharId, SetId, Letter, Label, Descr)"
                    + " VALUES"
                    + "     ( '" + charId.ToString() + "'"
                    + "     , '" + setId.ToString() + "'"
		...
		{
            if (string.IsNullOrEmpty(inputSet.Label.Trim()))
                Pause();
			...
			// ---------------------------------------------------------------------------
            //RichSqlCommand cmd = new RichSqlCommand(CommandType.Text, conn
            //    , "\r\n" + " INSERT INTO "+EndemeDataAccess.EnSchemaName+".EndemeSet"
            ...
            //    + "\r\n" + "        )", Throws.Actions, "P");
            RichSqlCommand cmd = new RichSqlCommand(CommandType.Text, conn
                , "\r\n" + " INSERT INTO "+EndemeAccess.LaEndemeSet+""
        ...
        //    Endeme enFound = acc.GetEndeme(enPrependTo, connection, trx);
        ...
        private static Endeme RetrieveEndeme_old(Guid endemeId, EndemeReference enRef, string conn)
        {
            ...
            // --------------------------------------------------------------------------
			RichSqlCommand cmd = new RichSqlCommand(CommandType.Text, conn
                , "SELECT e.*, s.Label AS SetName FROM "+EndemeAccess.LaEndemeMain + " AS e WITH(NOLOCK) LEFT OUTER JOIN EndemeSet As s WITH(NOLOCK) ON s.SetId = e.SetId WHERE EndemeId = @key"
                , Throws.Actions, "P")
                ._AddParameter("@key", endemeId);
            RichDataTable endeme = new RichDataTable(cmd, "Endeme", "EndemeID");
            ...
            // --------------------------------------------------------------------------
            Endeme en = new Endeme(enSet, endeme.StrValue(0, "CharString", ""), false);
		...
		public static Guid StoreEndemeSet_old(EndemeSet inputSet, string conn)
		{
			...
			if (chars == null || chars.Count == 0)
            {
                ...
                {
                    if (string.IsNullOrEmpty(characteristic.Code.Trim()))
