...
namespace InfoLib.Data
{
    ...
    public class Tally : Dictionary<string,int>
    {
        ...
        public Tally(char[] array) { for (int i = 0; i < array.Length; ++i) Incr(array[i].ToString()); }


        // ----------------------------------------------------------------------------------------
        //  Short methods and properties
        // ----------------------------------------------------------------------------------------
        public        double       Average    ()    { return ((double)TotalValue() / (double)TotalCount);                                                           } /// <summary>Keys: Returns a sorted list of keys (overrides Keys), sorts alphabetically, or numerically if you put them in with enough zero padding</summary>
        public new    List<string> Keys       ()    { List<string> list = new List<string>(this.Count); foreach (string key in base.Keys) list.Add(key); list.Sort(); return list; } /// <summary>Max: Returns the manimum (or final) key, works whether the numbers are labels or strings</summary>
        public        string       Max        ()    { List<string> key = Keys(); return key[key.Count-1];                                                           }   /// <summary>MaxTally: Returns the maximum tally</summary>
        public        int          MaxTally   { get { return this[Mode()];                                                                                          } } /// <summary>Min: Returns the minimum (or first) key, works whether the numbers are labels or strings</summary>
        public        string       Min        ()    { List<string> key = Keys(); if (key.Count > 0)  return key[0]; else return "";                                 }   /// <summary>Returns a string of stars or other characters</summary>
        public static string       Stars(int num, char cha) { string stars = ""; for (int i = 0; i < num; ++i) stars += cha; return stars;                          }
        public        List<string> ToList     ()    { List<string> list = new List<string>(); foreach (string key in this.Keys()) list.Add(key); return list;       }   /// <summary>TotalCount: Returns the total tally by adding up all the sub-tallys, works whether the numbers are labels or strings</summary>
        public        int          TotalCount { get { int total = 0; foreach (string key in Keys()) total += this[key]; return total;                               } }
        ...
        {
            if (this.ContainsKey(value)) this[value]++;
            else this.Add(value, 1);
        }
    }
