...
namespace InfoLib.SoftData // THIS NAMESPACE IS A PRIMITIVE!  use only System.* or InfoLib.Testing references
{
	...
	public class FuzzyTerm
	{
		...
		// -----------------------------------------------------------------------------------------
		public string Term { get { return _term; } } private string _term;
		...
		{
			if ((val1 < val2 & val2 < val3)) { return true; } else { return false; }
		}
		...
		{
			if ((measure <= Lowest) | (Highest <= measure)) { return 0.0;                                  }
            else if (Between(Lowest, measure, Low    )) { return (measure - Lowest ) / (Low     - Lowest); }
            else if (Between(High  , measure, Highest)) { return (Highest - measure) / (Highest - High  ); }
            else                                        { return 1.0;                                      }
		}
