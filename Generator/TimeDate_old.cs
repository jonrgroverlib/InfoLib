...
using System.Windows.Forms;           // for DateTimePicker
...
namespace InfoLib.Generator
{
    ...
    public class TimeDate_old
    {
        ...
        /// <summary>
        ///      Sets or gets the datetime in the CLR DateTime format (the native format of C#)
        /// </summary>
        ...
        /// <summary>
        ///      Sets or gets the datetime in the SqlDateTime format (the native format of SQL Server)
        /// </summary>
        ...
        // ----------------------------------------------------------------------------------------
        ///  Zero-input constructors: Sets time to a 'null' value
        // ----------------------------------------------------------------------------------------
        ...
        // ----------------------------------------------------------------------------------------
        ///  Three-input constructors: usually date, time, kind
        // ----------------------------------------------------------------------------------------
        ...
        /// <summary>for HL7 dates</summary>
        public TimeDate_old(DateTime datePart, bool specified, DateTimeKind kind)
        {
            if (specified) { CLRFormat = datePart; SetKind(kind); }
            else SetDateNull();
        }


        // ----------------------------------------------------------------------------------------
        ///  Five-input constructor: date, hours, minutes, seconds, kind
        // ----------------------------------------------------------------------------------------
        ...
        public bool Specified {
            get { return (!IsNull && this > TimeDate_old.MinSqlValue); }
            set { if (!value) SetDateNull(); } }
        public DateTime DatePart { get { return CLRFormat; } set { CLRFormat = value; } }
        ...
        private static DateTime ISOtoCLR(String isoDateTime)
        {
            ...
            // --------------------------------------------------------------------------
            int years   = _gen_.Integer(isoDateTime.Substring(0, 4), 1);
            int months  = _gen_.Integer(isoDateTime.Substring(5, 2), 1);
            int days    = _gen_.Integer(isoDateTime.Substring(8, 2), 1);
            int hours   = _gen_.Integer(isoDateTime.Substring(11, 2), 1);
            int minutes = _gen_.Integer(isoDateTime.Substring(14, 2), 1);
            int seconds = _gen_.Integer(isoDateTime.Substring(17, 2), 1);
        ...
        private static bool IsoNearlyEqual(String isoDateTime, DateTime clrDateTime)
        {
            ...
            equal = (Is.InOrder(isoTicks - offset, clrTicks, isoTicks + offset));
        ...
        private static TimeSpan TimeZoneToSpan(String isoTimeZone)
        {
            ...
            {
                int hours = _gen_.Integer(Regex.Replace(isoTimeZone, "^[-+](..):..$", "$1"), 0);
                int minutes = _gen_.Integer(Regex.Replace(isoTimeZone, "^[-+]..:(..)$", "$1"), 0);
                int seconds = 0;
